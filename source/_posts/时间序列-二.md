...
### 指数加权函数
另一个使用固定大小窗口及相等权数观测值的办法是定义一个衰减因子(decay factor)常量，以便使近期的观测值拥有更大的权数。*如果ma_t是时间t的移动平均结果，x是时间序列，结果中的各个值可用ma_t=a*ma_t-1 +(a-1)\*x\_-t进行计算，其中a为衰减因子。* 衰减因子的定义方式有很多，比较流行的是使用时间间隔( *span*)，它可以使结果兼容于窗口大小等于时间间隔的简单移动窗口函数:
* 简单移动平均与指数加权移动平均
```Python
