...
$$
\begin{cases}
    \max_{\boldsymbol{\alpha}}\sum_{i=1}\alpha_i - \frac{1}{2}\sum_{i,j=1}^m\alpha_i\alpha_jy_iy_j\boldsymbol{x}_i^T\boldsymbol{x}_j \\
...
$$
最后引入**松弛变量**(*slack variables*)$\epsilon_i\ge0=l_{0/1}y_i(\boldsymbol{w}^Tx_i+b)$即可得到**软间隔支持向量机**，将优化目标重写为:
$$
