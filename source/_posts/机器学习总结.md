...
E(f;D) &= E_D[(f(\boldsymbol{x};D)-y_D)^2] \\
       ...
       &=var(\boldsymbol{x})+bias^2(\boldsymbol{x}) + \epsilon^2
\end{aligned}
$$
...
$$
通过求解对偶问题，将原问题对$\boldsymbol{w}$的求解转化成对$\boldsymbol{\alpha}$的求解，将纬度从特征数量(和$\boldsymbol{x}$的属性一致)转变为样本数量
### 软间隔
...
$$
最后引入**松弛变量**(*slack variables*)$\epsilon_i\ge0$即可得到**软间隔支持向量机**，将优化目标重写为:
$$
\begin{cases}
    \min_{\boldsymbol{w},b,\epsilon}\frac{1}{2}||\boldsymbol{w}||^2+C\sum_{i=1}^m\epsilon_i\\
    \\
    s.t. \ y_i(\boldsymbol{w}^T\boldsymbol{x}_i+b)\ge 1-\epsilon_i \\
    \\
    \epsilon_i\ge0,\ i=1,2,\cdots,m
\end{cases}
$$
* 软间隔
![软间隔](机器学习总结/软间隔.png)
...
#### 软间隔的最优化
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;对每条约束$y_i(\boldsymbol{w}^T\boldsymbol{x}_i+b)\ge 1-\epsilon_i $添加拉格朗日乘子$\alpha_i\ge0$得$\alpha_i(1-\epsilon_iy_i(\boldsymbol{w}^T\boldsymbol{x}_i+b))$;对每条约束$\epsilon_i\ge0$添加拉格朗日乘子$\multimap_i$得$-\mu_i\epsilon_i$，最后得到拉格朗日函数为:
$$
L(\boldsymbol{w},b,\boldsymbol{\alpha},\boldsymbol{\epsilon},\boldsymbol{\mu})=\frac{1}{2}||\boldsymbol{w}||^2+C\sum_{i=1}^m\epsilon_i+\sum_{i=1}^m\alpha_i(1-\epsilon_iy_i(\boldsymbol{w}^T\boldsymbol{x}_i+b))-\sum_{i=1}^m\mu_i\epsilon_i
$$
令$L(\boldsymbol{w},b,\boldsymbol{\alpha},\boldsymbol{\epsilon},\boldsymbol{\mu})$分别对$\boldsymbol{w}$、$b$、$\epsilon_i$的偏导为零可得:
$$
\begin{aligned}
...
\end{aligned}
$$
将其代入$L(\boldsymbol{w},b,\boldsymbol{\alpha},\boldsymbol{\epsilon},\boldsymbol{\mu})$中可得对偶问题:
$$
\begin{cases}
